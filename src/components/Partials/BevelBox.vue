...
const handleBorders = () => {
  if (isVisible.value || init === false) {
    ...
    // Read data from global state, accounting for touch input.
    const mouseX = isTouch?.value
      ? window.innerWidth / 2
      : mousePos?.x?.value || 0;
    const mouseY = isTouch?.value
...
<template>
  <div
    ref="outerBox"
    class="bevel-box__outer bevel-box"
    :class="props.activeSides"
  >
...
/* The existing SCSS remains unchanged. */
.bevel-box {
  --b-width: 5px;
